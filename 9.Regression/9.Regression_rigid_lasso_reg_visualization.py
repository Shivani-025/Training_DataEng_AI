...
plt.legend()
plt.title('Ridge 9.Regression Fit')
plt.show()
...
plt.plot(X, predicted, label='Fitted line', color='red')
plt.legend()
plt.title('Lasso 9.Regression Fit')
plt.show()
...
# Calculate metrics for Ridge 9.Regression
model.eval()
with torch.no_grad():
    y_pred = model(X_tensor)
    mae, mse, rmse, r2 = calculate_metrics(y_tensor, y_pred)
    print(f'Ridge 9.Regression - MAE: {mae:.4f}, MSE: {mse:.4f}, RMSE: {rmse:.4f}, R-squared: {r2:.4f}')
# Calculate metrics for Lasso 9.Regression
model.eval()
with torch.no_grad():
    y_pred = model(X_tensor)
    mae, mse, rmse, r2 = calculate_metrics(y_tensor, y_pred)
    print(f'Lasso 9.Regression - MAE: {mae:.4f}, MSE: {mse:.4f}, RMSE: {rmse:.4f}, R-squared: {r2:.4f}')
